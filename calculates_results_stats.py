...
def calculates_results_stats(results_dic):
    ...
    """
    # statistic result dictionary
    results_stats_dic = {}

    # initialize count and pct variables
    n_dogs_img = 0
    n_notdogs_img = 0
    n_match = 0
    n_correct_dogs = 0
    ...
    # get number od images
    n_images = len(results_dic) 
    
    # update count variables
    for label in results_dic.items():
        # number of correct dog matches
        if label[1][3] and label[1][4]:
          n_correct_dogs += 1
        # number of dog images
        if label[1][3]:
          n_dogs_img += 1
        # number of correct non-dog matches
        if (not label[1][3]) and (not label[1][4]):
          n_correct_notdogs += 1
        # number of not dog images
        if not label[1][3]:
          n_notdogs_img += 1
        # number of correct breed matches
        if label[1][3] and label[1][2]:
          n_correct_breed += 1
        # number of label matches
        if label[1][2]:
          n_match += 1
    
       # catch error in case of zero division
    try:
        # update pct variables
    ...
    # update statistic result dictionary
