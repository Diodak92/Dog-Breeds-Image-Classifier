...
def get_pet_labels(image_dir):
    ...
    for immage_name in filenames:
      label = immage_name[:immage_name.index('.')].lower().replace('_', ' ').split()
      for word in label:
        if word.isdigit():
          label.pop(label.index(word))
      if immage_name not in results_dic:
        results_dic[immage_name] = [' '.join(label)]
